...
const Panel: React.FC<Props> = ({ material, isOpen, setIsOpen }) => {
  const context = useContext(Context);
  if (!context) throw new Error("Panel must be used within a View Component");

  const [byMaterial, setByMaterial] = useState<Data[]>([]);
    ...
    <Portal.Root>
      ...
      >
        <PanelComponent.Header>
          <p>Made of <span className="capitalize">{material}</span></p>
        </PanelComponent.Header>
        <PanelComponent.Content>
          <div className="grid grid-cols-4 gap-3 space-x-2 p-3">
            <Missing show={!!byMaterial} />
            {byMaterial.map((item, i) => (
